...
import { formatCardUpdatedAt, formatTimestamp } from "./status.ts"
...
test("formatTimestamp uses the locale-stable UTC stamp", () => {
  assert.equal(
    formatTimestamp("2026-09-11T21:38:00.000Z"),
    "Sep 11, 9:38 PM UTC"
  )
})
...
test("card footer stamp is Checked + relative age", () => {
  ...
  assert.equal(
    formatCardUpdatedAt(isoBefore(60 * 60_000), T0),
    "Checked 1h ago"
  )
  assert.equal(
