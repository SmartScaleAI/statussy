...
test("collectTransitions filters the pair list", () => {
  const items = collectTransitions([
    { serviceId: "openai", name: "OpenAI", previous: "operational", current: "major_outage" },
    { serviceId: "anthropic", name: "Anthropic", previous: "major_outage", current: "operational" },
    { serviceId: "groq", name: "Groq", previous: "partial_outage", current: "partial_outage" },
    { serviceId: "vercel", name: "Vercel", previous: "operational", current: "degraded" },
    { serviceId: "railway", name: "Railway", previous: null, current: "major_outage" },
    { serviceId: "github", name: "GitHub", previous: "degraded", current: "partial_outage" },
  ])
  assert.deepEqual(
...
test("groupUserDigests batches many favorite transitions into one digest per user", () => {
  const transitions = collectTransitions([
    { serviceId: "openai", name: "OpenAI", previous: "operational", current: "major_outage" },
    { serviceId: "anthropic", name: "Anthropic", previous: "operational", current: "partial_outage" },
    { serviceId: "vercel", name: "Vercel", previous: "operational", current: "partial_outage" },
  ])
  assert.equal(transitions.length, 3)

  const users = [
    { userId: "u1", email: "a@example.com", favoriteIds: ["openai", "anthropic", "vercel"] },
...
test("digestForUser ignores non-favorites and sorts Major before Partial", () => {
  const items = digestForUser(
    ...
    [
      { serviceId: "vercel", name: "Vercel", from: "operational", to: "partial_outage" },
      { serviceId: "anthropic", name: "Anthropic", from: "operational", to: "major_outage" },
      { serviceId: "openai", name: "OpenAI", from: "operational", to: "major_outage" },
    ],
  )
  assert.deepEqual(
    items.map((item) => item.serviceId),
...
test("digest body lists names, statuses, and a board link", () => {
  const items = [
    { serviceId: "openai", name: "OpenAI", from: "operational" as const, to: "major_outage" as const },
  ]
...
test("pairsFromSnapshotRows maps latest vs previous per service", () => {
  const { pairs, snapshotIds } = pairsFromSnapshotRows([
    { service_id: "openai", name: "OpenAI", snapshot_id: "10", status: "major_outage", rn: 1 },
    { service_id: "openai", name: "OpenAI", snapshot_id: "9", status: "operational", rn: 2 },
    { service_id: "vercel", name: "Vercel", snapshot_id: "5", status: "operational", rn: 1 },
  ])
  ...
  assert.deepEqual(
    ...
    [
      { serviceId: "openai", name: "OpenAI", previous: "operational", current: "major_outage" },
      { serviceId: "vercel", name: "Vercel", previous: null, current: "operational" },
    ],
  )
})
