...
export function digestServiceUrl(publicSiteUrl: string, serviceId: string): string {
...
export function pairsFromSnapshotRows(
  rows: LatestPairRow[],
): {
  pairs: Array<{
    serviceId: string
...
export async function sendStackDigests(
  ...
  const digests = groupUserDigests(groupSubscriberRows(subscriberRows), transitions)
  if (digests.length === 0) {
    return empty
  }
