...
Better Auth `user` / `session` / `account` / `verification`), seeds the 450 board services,
...
| --- | --- | --- |
| `DATABASE_URL` | Railway (worker, read/write) and Vercel (Next.js app) | Postgres connection string. The app reads live status, stores Better Auth users/sessions, and inserts footer **Suggest a Service** rows into `service_suggestions` (it does not write the `services` catalog). On Railway, reference the Postgres service (`${{Postgres.DATABASE_URL}}`, private network). On Vercel, use the Railway Postgres **`DATABASE_PUBLIC_URL`** — see [Point Vercel at Railway Postgres](#point-vercel-at-railway-postgres). |
| `BETTER_AUTH_SECRET` | Vercel (Next.js app) | Better Auth signing secret (at least 32 characters, e.g. `openssl rand -base64 32`). Never commit this value. |
| `BETTER_AUTH_URL` | Vercel (Next.js app) | Public app origin Better Auth uses for callbacks, e.g. `https://statussy.com` (or `http://localhost:3000` locally). Preview deploys should use that preview origin. |
| `RESEND_API_KEY` | Vercel (Next.js app) | Resend API key used to send email magic links. |
| `RESEND_FROM_EMAIL` | Vercel (Next.js app) | Verified Resend from address, e.g. `Statussy <noreply@statussy.com>`. |
| `GOOGLE_CLIENT_ID` | Vercel (Next.js app) | Google OAuth client ID. Authorized redirect URI: `{BETTER_AUTH_URL}/api/auth/callback/google`. |
| `GOOGLE_CLIENT_SECRET` | Vercel (Next.js app) | Google OAuth client secret. |
| `GITHUB_CLIENT_ID` | Vercel (Next.js app) | GitHub OAuth app client ID. Callback URL: `{BETTER_AUTH_URL}/api/auth/callback/github`. |
| `GITHUB_CLIENT_SECRET` | Vercel (Next.js app) | GitHub OAuth app client secret. Include the `user:email` scope. |
