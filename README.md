...
`service_snapshots`, `components`, `incidents`, `service_suggestions`), seeds the 261 board services,
...
Embrace), and Dash0 via `https://dash0status.com` (the public host
  ...
  (Beautiful.ai, Jitter via `jitter.statuspage.io`)
...
npm run migrate   # apply migrations + seed the 261 services, then exit
...
```ts
{
  id: "service-id",
  name: "Service Name",
  category: "ai", // or "cloud" | "developer" | "data" | "auth" | "payments" | "observability" | "email" | "design"
