...
`service_snapshots`, `components`, `incidents`, `service_suggestions`), seeds the 301 board services,
...
Embrace), and Dash0 via `https://dash0status.com` (the public host
  ...
  (`api.status.io/1.0/status/{pageId}`)
...
npm run migrate   # apply migrations + seed the 301 services, then exit
