...
`service_snapshots`, `components`, `incidents`, `service_suggestions`), seeds the 181 board services,
...
Dynatrace via Status.io (`api.status.io/1.0/status/{pageId}`), and
Better Stack via Better Stack `index.json`
...
npm run migrate   # apply migrations + seed the 181 services, then exit
