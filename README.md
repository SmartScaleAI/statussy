...
`service_snapshots`, `components`, `incidents`, `service_suggestions`), seeds the 221 board services,
...
Embrace), and Dash0 via `https://dash0status.com` (the public host
  ...
  Kit, Front, Omnisend), and SMTP2GO via `https://smtp2gostatus.com`
  (the public host redirects `/api/v2`)
...
npm run migrate   # apply migrations + seed the 221 services, then exit
