...
| `RESEND_FROM` | Vercel (Next.js app) and Railway (worker) | Verified Resend from address, e.g. `Statussy <noreply@statussy.com>`. `RESEND_FROM_EMAIL` is accepted as an alias. Same from-domain as magic link. **Colin may need to add this on the Railway worker.** |
