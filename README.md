...
informational notices do not paint the card), AWS via
`health.aws.amazon.com/public/currentevents` (UTF-16 JSON; empty list is
operational), Azure via `azure.status.microsoft/en-us/status/feed/`
(empty RSS is operational), Cloud Wave B Statuspage hosts
(Linode, bunny.net), Heroku via Status API v4 (`status.heroku.com/api/v4`;
Salesforce Trust is the public card URL), Fastly via StatusCast HTML on
`www.fastlystatus.com` (browser UA; worker UA is blocked), Deno Deploy and Koyeb via Instatus,
Modal via Better Stack `index.json`, Firebase via
`status.firebase.google.com/incidents.json`, Cloud Wave C Statuspage hosts
(Akamai, Scaleway, Lambda GPU Cloud), Northflank via Instatus, Vultr via
...
PayPal via RSS/Atom (`/feed/rss`), Adyen via its Nuxt `/api` JSON,
...
Authorize.net, Airwallex), and Payments Wave C Statuspage
...
host times out on `/api/v2`), Observability Wave A Statuspage hosts
(Datadog, Sentry, Grafana, New Relic, Honeycomb, Splunk Cloud, Axiom),
PagerDuty via `/api/services` + `/api/impacted_services` (empty
impact list = operational),
Dynatrace via Status.io (`api.status.io/1.0/status/{pageId}`),
Better Stack via Better Stack `index.json`, and Observability Wave B
Statuspage hosts (Sumo Logic, Coralogix, Rollbar, Bugsnag via
`bugsnag.status.smartbear.com`, incident.io, Mezmo, Airbrake, Cribl,
logz.io), Checkly via Checkly/Nuxt `__NUXT_DATA__` at
`https://is.checkly.online/` (catalog URL stays `status.checklyhq.com`),
Observability Wave C Statuspage hosts (Lumigo, Netdata,
Scout, Logit.io, Nobl9, Catchpoint, VictoriaMetrics, Langfuse,
Embrace), and Dash0 via `https://dash0status.com` (the public host
  redirects `/api/v2`), Email Wave A Statuspage hosts (Twilio, Mailgun,
  Klaviyo, Brevo, SparkPost, Braze, Loops, Mailjet), Resend via
  `https://resend-status.com` (the public host redirects), Customer.io
  via `https://customerio.statuspage.io` (the public host has no `/api/v2`),
  Email Wave B Statuspage hosts (Knock, Iterable, MailerSend, MailerLite,
  Kit, Front, Omnisend), SMTP2GO via `https://smtp2gostatus.com`
  ...
  (Figma, Canva, Miro, Webflow, Lucid, Mural, Frontify), Adobe via
  SnowServiceRegistry + StatusEvents, Sketch via Instatus, and Framer
  via Better Stack `index.json`, Design Wave B Statuspage hosts
  (Marvel, Balsamiq, Anima), Rive via Instatus, LottieFiles via the
  Checkly client-payload API (SSR is off — not `checkly_nuxt`),
  Whimsical via SorryApp HTML, Blender via Uptime Kuma, Design Wave C
  Statuspage hosts (Beautiful.ai, Jitter via `jitter.statuspage.io`),
  UXPin via `api.uxpin.com/status/services`, Infra Wave A
  Statuspage hosts (Terraform / Vault / Consul / Nomad / Packer via
  product-name filters on `status.hashicorp.com`, Pulumi, Chef,
  Spacelift via `spacelift.statuspage.io`, Crossplane via
  `status.upbound.io`), OpenTofu via Better Stack `index.json`,
  Infra Wave B Statuspage hosts (Puppet via `puppet.statuspage.io`,
  Scalr via `status.scalr.io`), Env0 via Instatus, Infra Wave C
  Statuspage hosts (Teleport via `status.teleport.sh`), Traefik via
  OpenStatus, Infracost via Better Stack `index.json`, Flags Wave A
  Statuspage hosts (LaunchDarkly, Optimizely, Statsig,
  Flagsmith), DevCycle via Status.io
  (`api.status.io/1.0/status/{pageId}`), Unleash and
  GrowthBook via Instatus, ConfigCat via StatusIQ RSS,
  Eppo via AdminLabs HTML (`status.eppo.cloud`), VWO via
  StatusIQ `/sp/api/public/summary_details`, Flags Wave B
  Statuspage hosts (CloudBees via `www.cloudbeesstatus.com`,
  Kameleoon via `kameleoon.statuspage.io`, Monetate via
  `monetate.statuspage.io`), Reflag and Hypertune via Instatus,
  AB Tasty via Sorry™ `/api/v1`, Convert via Pingdom Public
  Reports (`status.convert.com` — never `convert.statuspage.io`),
  Flags Wave C Statuspage hosts (Bloomreach), Omniconvert via
  Better Stack `index.json`, Flipper Cloud via Honeybadger HTML,
  Analytics Wave A Statuspage hosts
  (Amplitude, Mixpanel via `www.mixpanelstatus.com`, Segment,
  Heap, Pendo, RudderStack), Plausible via Better Stack
  `index.json`, PostHog via incident.io `/api/v1/summary`,
  Fathom via `status.usefathom.com/json`, Matomo via StatusIQ
  RSS, Analytics Wave B Statuspage hosts (Woopra,
  Treasure Data via `status.treasure.ai`, AppsFlyer, Branch,
  Hightouch via `status.hightouch.io`), Simple Analytics via
  the Jekyll incident diary, Parse.ly via StatusIQ RSS,
  Analytics Wave C Statuspage hosts (Singular, Airbridge,
  Polytomic, Baremetrics, Quantcast, Metabase, Hex), Lytics
  via the Contentstack `status.contentstack.com` Lytics group,
  Kissmetrics via Pulsetic `api.pulsetic.com/public/status`,
  ChartMogul via Statuspal HTML, Security Wave A Statuspage hosts (Snyk, Wiz,
  SentinelOne, Semgrep, Veracode, Tenable, Qualys, Vanta,
  Rapid7, Socket), Security Wave B Statuspage hosts (Aqua,
  Orca, Sysdig US-East-1, GitGuardian, Secureframe, Palo Alto Networks,
  ...
  Kustomer, Dixa, Kayako, Helpshift), Intercom Fin via
  `www.finstatus.com` (do not use `intercomstatus.com` —
  `/api/v2` returns HTML), Zendesk via `/api/ssp/services.json`
  and `/api/ssp/incidents.json`, Support Wave B Statuspage
  hosts (UserVoice, Ada, Plain, Forethought AI, Gladly via
  ...
  Stoplight via `stoplight.status.smartbear.com`, Swagger
  via `swagger.status.smartbear.com`, KnowledgeOwl via
  `knowledgeowl.statuspage.io` — the public host login-walls),
  Document360 via Admin Labs HTML on `status.document360.com`,
  Bump via Uptime.com on `status.bump.sh` (do not use
  abandoned `bump.statuspage.io`),
...
| --- | --- | --- |
| `DATABASE_URL` | Railway (worker, read/write) and Vercel (Next.js app) | Postgres connection string. The app reads live status, inserts footer **Suggest a Service** rows into `service_suggestions`, and stores Better Auth users/sessions (it does not write the `services` catalog). On Railway, reference the Postgres service (`${{Postgres.DATABASE_URL}}`, private network). On Vercel, use the Railway Postgres **`DATABASE_PUBLIC_URL`** — see [Point Vercel at Railway Postgres](#point-vercel-at-railway-postgres). |
| `BETTER_AUTH_SECRET` | Vercel (Next.js app) | Better Auth signing secret (at least 32 characters, e.g. `openssl rand -base64 32`). Required for login. Never commit this value. |
| `BETTER_AUTH_URL` | Vercel (Next.js app) | Public origin Better Auth uses for OAuth/magic-link callbacks, e.g. `https://statussy.com` (or `http://localhost:3000` locally). Callback URLs are `{BETTER_AUTH_URL}/api/auth/callback/google` and `/api/auth/callback/github`. |
| `RESEND_API_KEY` | Vercel (Next.js app) | Resend API key used to send email magic links. |
| `RESEND_FROM_EMAIL` | Vercel (Next.js app) | Verified Resend from address. Optional; defaults to `Statussy <noreply@statussy.com>`. |
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | Vercel (Next.js app) | Google OAuth Web client. Authorized redirect: `{BETTER_AUTH_URL}/api/auth/callback/google`. |
| `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET` | Vercel (Next.js app) | GitHub OAuth app. Authorization callback: `{BETTER_AUTH_URL}/api/auth/callback/github`. Include `user:email` (OAuth app) or Email addresses: Read-only (GitHub App). |
...
npm run migrate   # apply migrations + seed the 450 services, then exit
