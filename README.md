...
| `BETTER_AUTH_URL` | Vercel (Next.js app) | Fallback public origin (no trailing slash), e.g. `https://statussy.com`. Better Auth also accepts `statussy.com`, `www.statussy.com`, localhost, and `*.vercel.app` from the request host, so preview deploys work without a unique URL. Still set this in production. OAuth redirect URIs: `{origin}/api/auth/callback/google` and `{origin}/api/auth/callback/github`. |
