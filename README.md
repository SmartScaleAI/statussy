...
`service_snapshots`, `components`, `incidents`, `service_suggestions`), seeds the 341 board services,
...
Embrace), and Dash0 via `https://dash0status.com` (the public host
  ...
  Hightouch via `status.hightouch.io`)
...
npm run migrate   # apply migrations + seed the 341 services, then exit
