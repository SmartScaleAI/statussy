...
`service_snapshots`, `components`, `incidents`, `service_suggestions`), seeds the 291 board services,
...
Embrace), and Dash0 via `https://dash0status.com` (the public host
  ...
  (Teleport via `status.goteleport.com`)
...
npm run migrate   # apply migrations + seed the 291 services, then exit
